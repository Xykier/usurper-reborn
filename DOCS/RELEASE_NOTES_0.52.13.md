...
- `Scripts/Systems/CombatEngine.cs` — Biaxin handlers in all 3 combat paths (single-monster, multi-monster, PvP) using `IsCorroded`/`CorrodedDuration` for PvE and `StatusEffect.Weakened`/`StatusEffect.Poisoned` for PvP; Execute effect handler added to both single and multi-monster paths; Assassin Lethal Precision passive in both single and multi-monster attack paths; Arcane Shield spell failure/success messages in both combat paths; DEF display shows `(+X)` magic AC bonus in all 3 display modes (visual, BBS, screen reader)
- `Scripts/Locations/BaseLocation.cs` — Assassin Lethal Precision passive displayed in `/health` Active Buffs
